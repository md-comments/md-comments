...
import type { AnchorBlock, InlineComment, PageComment, Reply, Reaction } from '../../shared/types';
...
export class CommentsSidebarView extends ItemView {
  ...
  private renderResolvedCollapse(comment: any, type: 'inline' | 'page', quote = ''): string {
  ...
  private async handleClicks(e: MouseEvent) {
    ...
    if (tabBtn && tabBtn.dataset.tabId) {
      this.activeTab = tabBtn.dataset.tabId as any;
      await this.refresh();
      return;
    }
    ...
    if (action === 'submit-composer') {
      ...
      const compState = this.composers[key];
      delete this.composers[key];
      ...
      try {
        if (key === 'addPage') {
          ...
          const type = actionEl.dataset.type as any;
          ...
          await this.plugin.store.editComment(file, id, type, kind as any, rootId, text);
        }
      } catch (err) {
        console.error(err);
      }
    ...
    if (action === 'delete') {
      ...
      const rootId = actionEl.dataset.rootId!;
      const type = actionEl.dataset.type as any;
      const kind = actionEl.dataset.kind as any;
