...
1. `Chrome Extension Options UI` - 66 edges
...
## Knowledge Gaps
- **388 isolated node(s):** `root`, `parser`, `browser`, `node`, `es2022` (+383 more)
...
- **Are the 66 inferred relationships involving `Chrome Extension Options UI` (e.g. with `Chrome Extension Icon SVG` and `content.ts`) actually correct?**
  _`Chrome Extension Options UI` has 66 INFERRED edges - model-reasoned connections that need verification._
